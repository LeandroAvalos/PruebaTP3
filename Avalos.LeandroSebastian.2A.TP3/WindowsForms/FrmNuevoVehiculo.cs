...
namespace WindowsForms
{
    public partial class FrmNuevoVehiculo : Form
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(((TextBox)sender).Text) || ((TextBox)sender).Text.Length < 4 || !Regex.IsMatch(((TextBox)sender).Text, @"^[a-zA-Z0-9\s]+$"))
            {
                e.Cancel = true;

                epDatoDeLetrasIncorrecto.SetError((Control)sender, "El campo debe contener de 4 a 20 caracteres");
                epDatoDeLetrasIncorrecto.BlinkRate = 500;
                epDatoDeLetrasIncorrecto.BlinkStyle = ErrorBlinkStyle.AlwaysBlink;
            }
        }
